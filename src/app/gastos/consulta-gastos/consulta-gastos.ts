...
export class ConsultaGastos implements OnInit {
  ...
  cargarCategorias() {
    this.categoriasService.obtenerCategorias().subscribe({
      next: (categorias) => {
        ...
        console.log('Categorías cargadas en consulta-gastos:', categorias);
      },
      error: (error) => {
  ...
  exportarAExcel(): void {
    try {
      this.exportService.exportGastosToExcel(this.gastosFiltrados);
    } catch (error) {
  ...
  exportarAPDF(): void {
    try {
      this.exportService.exportGastosToPDF(this.gastosFiltrados);
    } catch (error) {
